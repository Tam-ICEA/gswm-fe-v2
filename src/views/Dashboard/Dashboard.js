    ...
    () =>[
      ...
      },
      {
        ...
        Cell: (props) => {
          ...
          {
            if(((Number(Date.now()) - Number(props.original.last_seen_at))/1000) <= (Number(info.reportPeriod)+60))
            {
              return <div style={{color: 'green', fontWeight: 'bold'}}>ONLINE</div>
            }
            else
            {
              return <div style={{color: 'red', fontWeight: 'bold'}}>OFFLINE</div>
            }
          }
      ...
      },
      {
        ...
        Cell: (props) => {
          ...
          {
            return <div>{props.original.denseData_diffirenceOfTheFlowPulseNumber.toString()}</div>
          }
          else
          {
            return <div>-</div>
      ...
      },
      {
        ...
        Cell: (props) => {
          ...
          if((happenedMagneticAttackStatus || happenedAntiDemolitionStatus) == true)
          {
            return <div style={{color: 'red', fontWeight: 'bold'}}>1</div>
          }
          else
      ...
      // <LinearProgress variant="determinate" value={normalise(props.value)} />
      {
        ...
        Cell: (props) => {
            ...
            {
                return <div><ProgressBar customLabel={Number(rsrp/10).toString()} completed={((Number(rsrp/10)+100)/3)+80} bgColor="green" labelAlignment="left"/></div>
            }
                
            if((Number(rsrp/10) >= -110)&&(Number(rsrp/10) < -100))
            {
                return <div><ProgressBar customLabel={Number(rsrp/10)} completed={3*(Number(rsrp/10)+110)+50} bgColor="#ffc107" labelAlignment="left"/></div>
            }
            if(Number(rsrp/10) <= -110)
            {
                return <div><ProgressBar customLabel={Number(rsrp/10)} completed={Number(rsrp/10)+160} bgColor="red" labelAlignment="left"/></div>
            }
      ...
      },
      {
        ...
        Cell: (props) => {
            ...
            {
                let remainingBatCap = ((8500*1000 - (((props.original.last_seen_at-props.original.install_time)*30)/(3600*1000)) - props.original.number_of_updates*1394.52)/(8500*1000))*100;
                return <div><BatteryGauge value={remainingBatCap} size={50} customization={{readingText:{fontSize:24}}}/></div>
            }
      ...
      },
      {
        ...
        accessor: 'last_seen_at',
          Cell: ( props ) => {
            ...
            if((props.original.last_seen_at == null)||(props.original.last_seen_at == ""))
            return <div>-</div>
            else
            return <div>{new Intl.DateTimeFormat('vi-VN', {year: 'numeric', month: '2-digit',day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit'}).format(props.original.last_seen_at)}</div>
           }
      },
      {
        Header: 'Voltage (V)',  
        ...
        <GridItem xs={12} sm={6} md={4}>
          <Card>
            <CardHeader color="success" stats icon>
              ...
              <h3 className={classes.cardTitle}>{deviceList?.filter(function (item) {return ((Number(Date.now()) - Number(item.last_seen_at))/1000) <= (Number(JSON.parse(item['/84/0']).reportPeriod)+60)}).length}</h3>
            </CardHeader>
            <CardFooter stats>
            </CardFooter>
          </Card>
          ...
          <Card chart>
            <CardHeader color="info">
              <ChartistGraph
                ...
                data={{
                  series: [
                  deviceList?.filter(function (item) {return ((Number(Date.now()) - Number(item.last_seen_at))/1000) <= (Number(JSON.parse(item['/84/0']).reportPeriod)+60)}).length,
                  deviceList?.filter(function (item) {return ((Number(Date.now()) - Number(item.last_seen_at))/1000) > (Number(JSON.parse(item['/84/0']).reportPeriod)+60)}).length
                  ],
                
                  labels: ["Online: " + deviceList?.filter(function (item) {return ((Number(Date.now()) - Number(item.last_seen_at))/1000) <= (Number(JSON.parse(item['/84/0']).reportPeriod)+60)}).length
                  , "Offline: " + deviceList?.filter(function (item) {return ((Number(Date.now()) - Number(item.last_seen_at))/1000) > (Number(JSON.parse(item['/84/0']).reportPeriod)+60)}).length
                  ],
                }}
                type="Pie"
